// Based on code from https://github.com/ethereum/go-ethereum/tree/38c7eb0f26eaa8df229d27e92f12e253313a6c8d/p2p/discv5
...
type (
	...
	findnode struct {
		Target     discv5.NodeID // doesn't need to be an actual public key
		Expiration uint64
		// Ignore additional fields (for forward compatibility).
		Rest []rlp.RawValue `rlp:"tail"`
	}
...
func nodeFromRPC(sender *net.UDPAddr, rn rpcNode) (*discv5.Node, error) {
...
func nodeToRPC(n *discv5.Node) rpcNode {
	return rpcNode{ID: n.ID, IP: n.IP, UDP: n.UDP, TCP: n.TCP}
}

type ingressPacket struct {
	remoteID   discv5.NodeID
...
func (t *udp) sendPing(remote *Node, toaddr *net.UDPAddr, topics []discv5.Topic) (hash []byte) {
...
func (t *udp) sendFindnode(remote *Node, target discv5.NodeID) {
...
func (t *udp) sendTopicRegister(remote *Node, topics []discv5.Topic, idx int, pong []byte) {
...
func (t *udp) sendPacket(toid discv5.NodeID, toaddr *net.UDPAddr, ptype byte, req interface{}) (hash []byte, err error) {
...
func decodePacket(buffer []byte, pkt *ingressPacket) error {
	...
	fromID, err := recoverdiscv5.NodeID(crypto.Keccak256(buf[headSize:]), sig)
	if err != nil {
		return err
	}
	pkt.rawData = buf
